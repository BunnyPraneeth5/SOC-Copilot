# Windows System Event Log Exporter
# Sprint-17: System Log Ingestion
#
# PURPOSE: Export Windows System Event Logs to file for SOC Copilot ingestion
# ARCHITECTURE: OS Logs → This Script → File → SOC Copilot
#
# USAGE:
#   powershell -ExecutionPolicy Bypass -File export_windows_system.ps1
#
# CONFIGURATION:
...
param(
    [string]$OutputFile = "logs\system\windows_system.log",
...
Write-Host "Windows System Event Log Exporter"
Write-Host "=================================="
...
while ($true) {
    try {
        # Get new system events since last check
        ...
        if ($Events) {
            ...
            foreach ($Event in $Events) {
                $LogLine = "{0}|EventID={1}|Level={2}|Source={3}|Message={4}" -f `
